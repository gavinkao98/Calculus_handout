...
| **§7.1 模板系統設計其餘部分**——版面 4 條規則、數學排版 5 條、**`worked_example` 新模板**、字體 | `worked_example`＝本對話（[`KICKOFF-worked-example-template.md`](KICKOFF-worked-example-template.md)，commit `c79372c`） | ✅ **`worked_example` 已落地**（2026-09-13，REBUILD_STATUS「工具線・`worked_example` 模板落地」條；報告 [`_audit/REVIEW-worked-example-template-applied.html`](_audit/REVIEW-worked-example-template-applied.html)）；版面 4 條進 sizecheck／數學 5 條的字級裁決／字體 ⚪ 仍未認領 |
