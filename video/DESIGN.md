...
meta:
  ...
  language: en
  theme: midnight
  voice: Mia                      # MiMo preset voice (fixed default; Gemini retired)
...
不開 L3，RAIL 被 reason 佔）；procedure/theorem/recap＝**L1＋L2 核心受益**（反應式→預測式）；
graph/value_table/sign_chart＝**明確排除精準路徑**（高度非線性：col_w 自適、axes 縮放、marks 寬度耦合，
硬套 metadata 會 false-positive；維持 `_overflow_issues`／`_graph_label_overlap_issues` 反應式）。
...
每模板 fit+over 各一場景 ＋ [`video/capacity_selftest.py`](capacity_selftest.py) 斷言 EXPECT 表，10/10 通過；
本地跑（需 manim、非 handout CI），改模板若破壞容量預測即紅】**
...
**`aside` 欄位契約（L3）：** `definition_math`（未來可推廣 callout/theorem statement-only）的可選 `aside`：
`{label?, body, accent?}`（或裸 body 字串）。有 `aside` 時 primary 收窄到 `PRIMARY_W`、卡片 snap `RAIL_X`
（`build_aside` 走 `accent_panel`、`layer="decoration"` 豁免容量/overlap 檢查）；primary 收窄後若會溢出
zone 或某 math 列寬過 `PRIMARY_W` 則**自動收合**（丟 aside、回滿寬單欄）。**author-authored、純可選——框架絕不
自動生成內容填空**（否則退化成被否決的 auto-fit 投機）。
...
> **V1–V8 blocking（收斂＝視覺 blocking==0）＋A1–A7 magnitude（0–100，驅動重 render 優先序）**。
...
它提取每場景的 fullest frame，讓 vision model（MiMo-V2.5）對照 **VISUAL-FRAME-RUBRIC（V1–V8 blocking＋A1–A7）**
...
- MiMo specifics（唯一 TTS 路線，2026-06-16）：model `mimo-v2.5-tts`、voice
  `meta.voice` / `Mia`、OpenAI 相容 `/chat/completions`（與 `critic.py` 共用
  `MIMO_API_KEY`）；待唸文字放 `assistant`、風格放 `user`；回 WAV，`MimoTTSBackend`
