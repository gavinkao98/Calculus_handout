...
- **模組：** 核心 `pipeline/scene_align.py`（純函式＋單一 aligner seam `align_scene`，換 aligner 只改這函式）；fallback ladder `pipeline/scene_fallback.py`；`tts.py --unit beat|scene|auto`（auto 依 batch-1 template allowlist）。**batch-1 ladder＝arbiter(small.en 免費)→resynth(計費)→beats(免費終點)；rung 3 sentence-chunk 暫緩至 batch-2**（pilot 三類場景 FA 全過、幾乎不觸發）。
