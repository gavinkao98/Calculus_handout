...
- **reuse 以內容定址、不以輸出路徑定址（2026-09-13 修）：** beat 級 reuse index 的 key＝`(scene_id, beat text_hash, 同場同 hash 的第幾個)`＋頂層 backend/model/voice/style（`build_reuse_index`）；scene 級沿用 `scene_id`（`build_scene_reuse_index`，本來就對）。**理由：** beat 輸出路徑是 `beats/<場號>_<scene_id>/<序>_<reveal>.wav`，場號與 reveal 都進檔名，所以「搬場」或「加／移一個 `{show}` marker」會讓路徑全部位移——舊的路徑 key 於是判定「無先前紀錄」而把整場重合成，即使一個字都沒改（2026-09-13 兩次踩到）。命中後若新舊路徑不同就把 WAV **搬**到新路徑再登記（copy→驗大小→刪舊→空目錄剪掉；log `reused … (moved from …)`），舊路徑不留孤兒 WAV 去絆 `overwrite_guard`。同場同文字的兩拍依 index 配對、同一份 take 只登記一次。
