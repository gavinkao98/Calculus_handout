...
> ✅ **字體已定案：全 Computer Modern**（與設計師 `tokens.json` 原案刻意不同，紀錄如下）：
> - **設計師 `tokens.json` 原案**：display = `Space Grotesk`、body = `Hanken Grotesk`、mono = `JetBrains Mono`（現代 sans）。
> - **最終實作（成片實際使用）**：非數學文字**全部統一為 Computer Modern**（`CMU Serif` / mono `CMU Typewriter Text`），與數學 LaTeX 同字族。
> - **定案理由**：body 與 math 同族（一致的襯線學術感）、字型檔可內嵌進 repo（無需 Google Fonts 連網）、避免 sans body font 缺 `✓ ✗ ∎` 等符號的 tofu 問題。
> - **給設計師**：請在「全 CMU」前提下精修（字級、間距、層次）；sans 提案已不採用，毋需再評估其與 LaTeX 的搭配或內嵌方式。
