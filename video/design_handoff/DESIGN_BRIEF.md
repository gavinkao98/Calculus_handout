...
> ⚠️ **這裡有一個待你定案的分歧**——設計 token 與現行實作不一致：
> - **設計師 `tokens.json` 原案**：display = `Space Grotesk`、body = `Hanken Grotesk`、mono = `JetBrains Mono`（現代 sans）。
> - **現行實作（成片實際使用）**：非數學文字**全部統一為 Computer Modern**（`CMU Serif` / mono `CMU Typewriter Text`），與數學 LaTeX 同字族。
> - **為何工程端改了**：body 與 math 同族（一致的襯線學術感）、字型檔可內嵌進 repo（無需 Google Fonts 連網）、避免 sans body font 缺 `✓ ✗ ∎` 等符號的 tofu 問題。
> - **要設計的事**：請確認方向——維持 Computer Modern serif（現狀），或回到 sans 提案（需重評與數學 LaTeX 的搭配與字型內嵌方式）。
...
### 另一個現狀分歧（座標格線）
`tokens.json` 把 80px 的 blueprint 座標格列為背景 motif，但**目前所有模板都停用格線**（`theme.py` `SHOW_GRID = False`，視覺決定要乾淨背景）；`grid_line` 色保留備用。若你希望「Blueprint Grid」名副其實地露出格線，這是另一個可討論點。
...
| `08-outro` | outro（淺·彩色 lockup + Key Takeaways） | outro | `templates/outro.py` |
...
1. 先校準 **A 全域視覺語言**（特別是字體分歧——這影響每一個模板）。
...
> 暗底教學 + 淺底品牌（intro/outro），真實數值在 `from_designer/handoff/tokens.json`。重點待辦：**字體定案
> （sans token vs Computer Modern serif 實作）**、雙 palette 對比校準、各模板版面微調。數學式一律 LaTeX 佔位
> （Computer Modern），所有動畫由工程端 Manim 實作（別做 CSS/JS 動畫）。現行截圖見
> `from_designer/handoff/screenshots/`。
