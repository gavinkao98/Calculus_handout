...
- **D-P3-6 — calibrate on a fixture, never a locked deck (§11 overfit caution).** Calibrate on a purpose-built fixture (storyboard + backing `.md`) with planted PD/OF blocking + clean + boundary cases; never calibrate by editing a real/locked deck.
...
4. **OF1–OF2 — on-screen-text faithfulness (§5, §7 ii).**
   - **OF1** faithfulness (blocking, **agent judgment**): on-screen teaching text **contradicts or exceeds** its cited source (adds math/conclusions the source lacks; changes a condition). OF1 **reads the real cited source**, not just whether a ref exists. **Source-adequacy (D-P3-4):** also flag when the cited source is **too broad to specifically support the claim** (e.g., a whole-section / recap-synthesis source cited for a specific sub-claim it does not state) → a field-level `source:`/`from:` override is required. OF1 does **not** re-compute math correctness (that is `L5`).
   - **OF2** traceability (blocking, **structural/deterministic**): missing provenance ref → `provenance.py` (Plan 1). The agent surfaces it; it does not re-implement it.
   - **Field-level override rule (§5.1):** all on-screen text fields inherit the scene-level `source`; a field-level override is required only when the field (a) cites a *different* source, (b) synthesizes across units, or (c) makes a high-risk math claim. OF1 source-adequacy enforces this.
...
# 你要審什麼（一次讀齊）
1. **storyboard**：`video/storyboards/<deck>.yml`（場 kind/template、scaffold.motive/problem/flag、meta.pedagogy_profile、meta.assumptions、各場/欄的 `source`/`ref`/`refs`）。
...
# 怎麼做
- 按 rubric 走 **PD1–PD4 ＋ OF1–OF2**，**PD blocking 與 OF blocking 分開計數**（rubric 硬紀律）。
- **gate-1 自有 blocking ＝ PD1（一 beat 多動作）＋ OF1（上畫面文字矛盾/超出/源不夠 specific）**；PD2/3/4 結構與 OF2 是確定性層（`pedagogy.py`/`provenance.py`）算的，你**surface＋給教學脈絡**、不重算。
- **OF 生命週期：** cited 源 `CONTENT_APPROVED≠yes` → OF 一律 **dry-run**（報為 advisory/dry-run、不計 blocking）。
- 嚴守 §10 邊界：**絕不** raise `L1/L2/L6`（六鏡）、`NFA`、`V*/A*`（視覺）、`E1`（hook）已擁有的 finding。
- 每條 blocking 必 cite：unit id ＋ 確切 beat/欄位 ＋ 缺的步驟或矛盾點 ＋ 一個最小修法。**不 over-report**——乾淨維度是有效結果。**禁止**自動改寫迴圈、**禁止** re-litigate 已認可教學法。
...
**Files:**
- Create: `video/storyboards/_fixtures/pedagogy_audit.yml`
- Create: `video/content_scripts/_fixture_pedagogy.md`
- Create: `video/content_scripts/_audit/CALIBRATION-pedagogy-firstlearner.md`

**Interface:** a self-contained fixture that plants exactly one of each blocking class + clean + boundary cases, with an expected-findings table the calibration (Task 4) diffs against. The fixture is the "test" for a judgment gate.

- [ ] **Step 1: Create the backing `.md` (cited sources)**

Create `video/content_scripts/_fixture_pedagogy.md` with `CONTENT_APPROVED: yes` at the deck level and a few units, e.g.:
- `unit_squeeze` (APPROVED via deck) — states the squeeze bound `cos θ ≤ sinθ/θ ≤ 1`, used as the faithful OF1 source.
- `unit_narrow` (APPROVED) — a *narrow* source that does NOT state a broader claim (for the OF1 source-adequacy / exceeds-source plant).
- Plus a DRAFT sibling fixture note: a second `.md` (`_fixture_pedagogy_draft.md`, `CONTENT_APPROVED: no`) cited by one scene, to exercise OF dry-run.

(Keep units minimal but real prose, so OF1 has actual source text to judge "supported / contradicted / exceeded".)

- [ ] **Step 2: Create the fixture storyboard with planted + clean + boundary scenes**

Create `video/storyboards/_fixtures/pedagogy_audit.yml` with `meta.pedagogy_profile: first_time`, an `assumptions` registry, and scenes planting:
- **PD1 blocking:** a `derivation` scene whose single `{show}` beat compresses ≥2 load-bearing algebra steps into one reveal.
- **PD1 clean:** a `derivation` scene that segments the same work across beats (must NOT be flagged).
- **PD2 advisory:** a `theorem_proof` scene that HAS `scaffold.motive` but the motive is weak/empty-feeling (deterministic PD2 is satisfied; the agent advisory should note weakness). *(Deterministic missing-motive is already covered by `pedagogy.py`; do not double-plant it as the agent's blocking.)*
- **PD3 advisory:** a `kind: divider` with a `scaffold.problem` that is vague/non-concrete (deterministic PD3 satisfied; agent advisory).
- **OF1 blocking (exceeds source):** a scene whose on-screen `statement` asserts more than its cited `md:unit_narrow` states (added conclusion) — with `CONTENT_APPROVED=yes`, so OF1 is blocking.
- **OF1 blocking (source-adequacy):** a scene citing an over-broad scene-level source for a specific sub-claim that the source does not specifically support (missing field-level override).
- **OF1 dry-run:** a scene citing the DRAFT `.md` (`CONTENT_APPROVED=no`) with an exceeds-source text — expected **dry-run/advisory, NOT blocking** (lifecycle).
- **OF1 clean:** a scene whose text is fully supported by its cited source.
- **Boundary (must NOT raise):** (i) a unit with two distinct teaching *concepts* (that is `L2`, not PD1); (ii) an `intro` scene with a tagline (that is `L6`); (iii) a `recap`/`recap_cards` scene (excluded by kind/template). The gate must stay silent on these.

- [ ] **Step 3: Create the expected-findings table**

Create `video/content_scripts/_audit/CALIBRATION-pedagogy-firstlearner.md` listing, per planted scene: the expected `code` (PD#/OF#), `Blocking|Advisory|dry-run`, and the cite. Add an explicit **MUST-NOT-RAISE** list (the three boundary scenes + the clean scenes) and the **lifecycle expectation** (the DRAFT-cited scene → OF dry-run, not blocking). Leave a "Calibration run result" section empty for Task 4.

- [ ] **Step 4: Commit**

```bash
git add video/storyboards/_fixtures/pedagogy_audit.yml video/content_scripts/_fixture_pedagogy.md video/content_scripts/_fixture_pedagogy_draft.md video/content_scripts/_audit/CALIBRATION-pedagogy-firstlearner.md
git commit -m "test(pedagogy): calibration fixture + expected-findings table for the gate"
```
...
**Interface:** the agent (following the rubric) produces findings that match the expected table — catches every planted blocking, raises none of the MUST-NOT-RAISE cases, keeps PD/OF counts separate, and dry-runs OF on the DRAFT-cited scene.

- [ ] **Step 1: Dispatch the gate-1 agent on the fixture**

Dispatch `subagent_type: pedagogy-firstlearner-audit` with the fixture paths (`storyboards/_fixtures/pedagogy_audit.yml`, the backing `.md`s, `CONTENT_APPROVED` states). Capture its `VERDICT:` line + findings.

- [ ] **Step 2: Diff findings against the expected table**

For each expected blocking: present? cited correctly? For each MUST-NOT-RAISE: absent? PD and OF counts separate? DRAFT-cited scene → OF dry-run (not blocking)?

- [ ] **Step 3: Iterate until calibrated**

If the agent misses a planted blocking, over-reports a boundary/clean case, merges PD/OF counts, or gates a DRAFT-cited OF: tighten the **rubric** (the contract) and/or the **agent body**, then re-dispatch. Calibrated = findings match the expected table (catch all planted, raise none forbidden, counts separate, lifecycle respected). Record the final run (verdict + diff) in `CALIBRATION-pedagogy-firstlearner.md`.
...
git commit -m "test(pedagogy): calibrate pedagogy-firstlearner gate against fixture (PD/OF separated, lifecycle, boundaries)"
```
