...
- **🖥️ 環境確認（2026-06-17，免再逐次查）：** manim 產線在本機**可跑**——**全域 `python`（Python 3.12）** 已裝 **manim 0.20.1 ＋ PyYAML 6.0.3**，**MiKTeX**（latex／dvisvgm）＋**系統 ffmpeg 8.x** 在 PATH；`.venv` 未裝 manim、`.deps*` 不存在，故 render／sizecheck/mock **一律用全域 `python` 跑**（非 `.venv`、不需 ffmpeg_shim）。已用 `python video\pipeline\sizecheck.py _demo_value_table.yml` 實跑驗證通過（manim 建場景＋LaTeX＋dvisvgm 全通、exit 0）。**此前本檔／README 記「manim 仍未裝」是當時該機狀態，現已更新；詳見 README §環境的「本機現況」註。**
