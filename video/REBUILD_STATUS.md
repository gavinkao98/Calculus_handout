...
**觸發：** 使用者宣告 ch1 HTML 講義定稿（[`chapter1-print-standalone.html`](../experiments/handout_kit/chapter1-print-standalone.html)，當日 import 後重生），啟動「整章做影片」。本節為跨對話錨點。
...
3. **ch01 內容權威檔 = [`chapter1-print-standalone.html`](../experiments/handout_kit/chapter1-print-standalone.html)**（2026-06-13 重組後檔名；原 `chapter1-standalone.html` 已不存，編輯源在 `fragments/ch01/sec-*.html`，影片產線讀組裝後的 standalone；非 example-ch01/ 片段；ch02+ 權威檔屆時另定）。
