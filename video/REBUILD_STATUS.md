...
- **Advisory 處置（使用者選「先修 template bug 再進 hook」，已修＋回歸）：** A 面 advisory **未經三票複驗**（只 V-blocking 走複驗），逐項調查：
  ...
  - **(b) `graph_focus` line label 顏色＝已修（template 根因）：** [`pipeline/templates/graph_focus.py`](pipeline/templates/graph_focus.py) `line` label 預設 `role="warning"`（紅）→ 改**繼承該線 `color_role`**（與 function label 一致）。blast radius 安全：唯二受影響＝ch01 兩條 `color_role:muted` 的 `y=x` 線（正是要修的）；所有 demo 的 line label 都設顯式 `label_role`、default 改了不受影響（py_compile 過、推理確認 no-op）。
