...
video/
  ...
  storyboards/
    ch01_inverse_functions.yml   §1.1（16 場景）
    ch01_precise_limit.yml       §1.6（16 場景，symbol-heavy ε-δ）
  output/              render 成品 + critic 報告（gitignored）
```
...
`output/critic/<id>/critique.{json,md}`，不改 storyboard；採納與否由人判斷。
