# KICKOFF — 品質補強輪 ⑰：色軸統一＋鋪滿輪 backlog 派工（2026-09-13）
...
1. **色軸照 11／18／24 的定：** 藍 `secondary`＝cos、琥珀 `accent`＝sin、綠 `success`＝斜率／加速度／tan；θ 物件（弧、扇形）維持色表 `concept` 赭。**08 改掉**（06／07 跟著對齊）。已知取捨：accent 與 concept 色相相近，θ 弧旁的 sin 半弦要靠明度分辨；若要拉開只能換 θ 的色（下一個裁決點，本輪不動）。
...
主模型（本對話）：審 A–D 的回報與幀、merge、解衝突、跑 main 的 selftest／smoke 對照、記 REBUILD_STATUS ⑰。另一 session（`calculus-handout-a7`）正在 main 做 Task D（計費改稿＋重 render），hooks 檔有它的 dirty hunk：A／D 的 merge 等它 commit 後做。
