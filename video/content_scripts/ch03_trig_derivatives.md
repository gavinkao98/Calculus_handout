...
> **source_rev：** `legacy/html_handout/fragments/ch03/sec-3-1.html` `sha256:92c92169bdd14d4fd463c11478cdc894c949a0aaa069dbbce39230c22e9eb759` — lock 時（2026-06-29，commit `5f3e7ea`）該 fragment 的 LF 正規化 sha256（2026-09-12 依產線評估 F2 補蓋；產生器 `python video/pipeline/source_rev.py <源檔>`）。schema／make／derive preflight 比對現檔，不符即 `[source_rev]` WARN＝「講義已變」→ 走 [`../CONTENT_METHODOLOGY.md`](../CONTENT_METHODOLOGY.md) §8（diff→外科修改→scoped NFA）後重蓋。**現況：lock 後講義已改三輪（2026-07-03 難度緩解／07-06 revise／07-26 平實化），且 2026-08-09 起唯一源＝`handout/latex/src/ch03/chapter3.tex`——§8 對齊待品質補強試點（§3.1 整節重做）併做；對齊後改 stamp 該 `.tex`。**
