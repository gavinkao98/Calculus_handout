...
- **source:** `chapter1-print-standalone.html` §1.6 · opener prose（history／foundations + 「close」須精確化，L1664–1666）＋ `expansion:application` 工程公差段（`sec-1-6.html` L14–15；2026-06-16 SL-1 folded as Incorporative，見漂移修復筆記）
- **learning_goal:** 體會「f(x) 靠近 L」這種口語不夠用，本節要把「靠近」換成可量的距離。
- **kind:** `motivation`
- **narration:**
  > For two centuries calculus solved real problems — orbits, areas, the motion of the planets — while its logical foundations stayed shaky. The arguments leaned on infinitesimals, quantities imagined as vanishingly small but never pinned down, and on vague phrases like "$x$ is close to $a$" and "$f(x)$ is close to $L$." But how close is close? The nineteenth century answered by throwing the word "close" out entirely and replacing it with a measurable distance. That single move is the whole project of this section: make "close" precise enough to compute with. And this is not abstract pedantry — it is the language engineering already speaks: a *tolerance* is exactly how far a finished quantity may stray from its target, an output tolerance around the value you want, met by an input tolerance you control.
...
- **source:** `chapter1-print-standalone.html` §1.6 · Strategy 1.4 [Verifying a limit from the ε-δ definition]（L1750–1761）
...
10. **provenance 全面重指＋記錄環境編號。** 14 個既有單元的 `source:` 自 `.tex` 行號改指 HTML §1.6＋手寫編號：**Definition 1.13**（ε-δ limit）、**Definition 1.14**（infinite limit）、**Proposition 1.7**（Uniqueness of limits）、**Strategy 1.4**（ε-δ 驗證；2026-06-16 六鏡訂正：原誤記 Strategy 1.3，與 §1.5 的真實 Strategy 1.3「Computing a limit」撞號）、**Figure 1.21**（fig:precise-limit）、**Example 1.37/1.40/1.41**。narration 主幹零刪改（既有單元逐字保留）。
...
### 六-lens 全節稽核（2026-06-16，Workflow `wf_5a8ec085-4ef`，全 19 單元）

繼 2026-06-14 的 scoped 稽核（只審漂移新材料）之後，**首次對全 19 單元跑完整六鏡**，用新 SSOT 契約 [`_audit/CONTENT-SIXLENS-RUBRIC.md`](_audit/CONTENT-SIXLENS-RUBRIC.md)（六鏡並行 → refute-by-default 逐條複驗），對照定稿 HTML §1.6。**裁決：1 blocking、2 advisory（去重自 1 blocking＋3 advisory 的跨鏡重複；1 refuted）。**

- **clean（3 鏡）：** L3 register（§4 口語標準無違規）、L4 no-repeat（u14/u16 確走 repeat-pattern）、**L5 math（0 錯——獨立盲算 u4 容差 0.05、u5 δ=ε/2、u9–u10 唯一性三角不等式、u12 常數、u13 δ=ε/4、u14 δ=min{1,ε/4}、u16 M-δ δ=1/√M 全通過）**。
- **SL-1〔Blocking，L2/L6〕engineering-tolerance enrichment 散文 silent drop：** 講義 §1.6（`sec-1-6.html` L14–15）有一段 `expansion:application` 把 ε／δ 讀成工程／製造公差（output tolerance ε／input tolerance δ、「control the input closely enough, and the output stays within specification」），未折疊進任何單元、亦未就近註明刻意略過（同節 `expansion:intuition` challenge-response 段已正當折進 u6）。解法二擇一：(a) 折進 u2 或 u6 一句 Incorporative lead-in；(b) 就近註明刻意略過。**已裁決→(a)（2026-06-16 使用者選「折進旁白」）：** 工程公差詮釋已折成 u2 一句 symbol-free Incorporative lead-in（刻意不用 $\varepsilon/\delta$ 符號、避免前向引用，因 ε/δ 要到 u5/u6 才引入），u2 source 欄就近註明。**回歸：** fold 保語意、補足遺漏 beat、未動任何數學；narration HTML u2 同步更新（標「SL-1 補」）待旁白認可。silent-drop 解除。
- **SL-2〔Advisory，L1/L6〕u11 provenance 編號錯——已修：** source 與本筆記 item 10 原記「Strategy 1.3」，但權威 env-num 為 **Strategy 1.4**（且「Strategy 1.3」是 §1.5「Computing a limit」的真實環境、構成撞號）。本輪已訂正 u11 source 欄與 item 10。其餘編號（Def 1.13/1.14、Prop 1.7、Fig 1.21、Ex 1.37/1.40/1.41）抽驗無誤。
- **refuted（1）：** u9 `kind: theorem` vs `proposition`——駁回。Prop 1.7 在講義帶證明，適用 §3「env-theorem＋env-proof → theorem(+proof)」列；且 §3 對該家族明文允許 theorem／proposition latitude（「教學角色不同而已」）。標 theorem 反而更貼對應表。
- **交付：** 審核稿 [`_audit/REVIEW-ch01_precise_limit-sixlens.html`](_audit/REVIEW-ch01_precise_limit-sixlens.html)（雙擊即開、數學即渲染）。**回歸：** SL-2 為純編號訂正、未動 narration／數學；SL-1 待裁決後再回歸。
