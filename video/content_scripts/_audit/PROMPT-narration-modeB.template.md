...
**D7 — Math-content correctness {{CONTENT_APPROVED}}=no → REQUIRED; =yes → only flag outright errors.** Independently re-derive every numeric value, sign, interval endpoint, quadrant, and final result in the narration; confirm each matches the mathematics (not merely the spoken-vs-written equivalence of D3). Quote any discrepancy.
