...
> **PD2/PD3/PD4 的結構 blocking 由 `pipeline/pedagogy.py`（Plan 2）確定性計算；gate-1 agent 帶教學脈絡把它浮現出來、並擁有 advisory 層。gate-1 agent 自己的 blocking ＝ PD1 ＋ OF1。**
...
OF findings **只有當 cited content-script 的 deck 級 `CONTENT_APPROVED=yes`（已 lock）時才是 blocking 類**；DRAFT 階段（`CONTENT_APPROVED=no`，源未定）OF 一律 **dry-run／advisory**（findings 報為 advisory／dry-run，**永不 blocking**）——否則是拿未核准的源當忠實基準。**deck→unit 繼承：** `md:<unit>` ref **繼承 deck 級 `CONTENT_APPROVED`**；`doc:<handout-anchor>` ref **一旦解析即可 gate**。
