...
- **Document the AS-BUILT, not the spec's wish-list.** Plans 1–4 landed; some spec details shifted in implementation. The landed code/rubrics are the source of truth where they differ from the spec's prose. Examples the implementer MUST honor (Codex-verified against the code):
  ...
  - the OTF provenance fields are **new `ref:` (scene-inherited) / `refs.<path>` (column override, e.g. `refs.scaffold.motive`, `refs.scaffold.problem`)**, grammar `md:<unit_id>` / `doc:<frag-sec-*|data-fig>`, **場級繼承 + 欄級覆寫**, warn-only + intro/outro 豁免 — **`source:` is a freeform human label, NOT parsed for provenance** (`provenance.py`);
...
- [ ] **Step 2: Add the scaffold 承載 contract section.** A new section (繁中, in-voice, e.g. `## Scaffold 承載與視覺契約`) after the Lectern section, documenting SPEC §6 + the as-built `render_scaffold`:
  ...
  - **scaffold provenance 走 OTF 的 `ref:`/`refs:`**（場景 `ref:` 繼承，或欄級 `refs.scaffold.motive`/`refs.scaffold.problem` 覆寫）；`source:` 為 freeform 人讀標籤、**不被解析為 provenance**（`provenance.py`）。指向 [`CONTENT_METHODOLOGY.md`](CONTENT_METHODOLOGY.md) 的 OTF 規則。
