...
本書由**不同教師撰寫的手稿**組裝而成。每章 entry 在 **Manuscript source** 欄位記錄其手稿來源，使草稿的起源與轉換狀態一目了然。手稿轉 LaTeX 的工作流程以及規範 Claude 擴展手稿的 anti-hallucination 規則都在 [`README.md`](README.md) §*Authoring workflow* 中；本檔案是各章手稿追蹤的落點。
...
**Notation introduced**
- 新符號、macro 或記號慣例。每一個在首次使用時都需要 `\index{...}`。
...
**Status**: draft
**Source file**: [`chapters/ch01_foundations.tex`](chapters/ch01_foundations.tex)——檔名 slug `foundations` 是弧線層級的標籤（Chapter 1 是弧線的 *foundations* 階段），不是印刷的章節標題的一部分。
**Estimated length**: *（完成首次完整編譯後填入）*
**Manuscript source**: pre-existing LaTeX——entry 從已提交的 `chapters/ch01_foundations.tex` 反推而來。以 LaTeX 原始碼為準；本 entry 是對現有內容的描述，不是未來草擬的計畫。當 Chapter 1 進行進一步編輯時，同時更新 LaTeX 和本 entry。
...
**Open questions**
- 全部六節的 end-of-section exercises 仍為 `% TODO` placeholder。見 [`CONTENT_EXERCISES.md`](CONTENT_EXERCISES.md) 中的最低習題骨架，在宣告本章 `done` 之前應先落地。
...
**Status**: draft——LaTeX `chapters/ch02_derivatives.tex` 尚未建構。手稿覆蓋在 **HTML handout-kit experiment** 中已**完成**（`experiments/handout_kit/exp-ch02/`）：§2.1–§2.5 透過 `direction_layer` six-stage flow 撰寫，每節經 Codex audit 至 blocking=0（2026-06）。
...
**Status**: manuscript coverage complete（§3.1–§3.3，handout kit，2026-06-08）——三節全部通過 six-stage convergence gate，包含 two-model adversarial audit（Claude multi-lens + Codex gpt-5.5 xhigh）。LaTeX `chapters/ch03_chain_rule.tex` 的建構是獨立的下游任務。
...
- Domain restriction 在適用於單一公式時以 inline 方式寫在 `conditiondisplay` 中；容易忘記時則移入 `caution`。
