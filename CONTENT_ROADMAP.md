...
**Status**: draft。手稿覆蓋在 HTML handout 中已**完成**（`handout/fragments/ch02/`）：§2.1–§2.5 透過六階方向層流程（[`CONTENT_DIRECTION.md`](CONTENT_DIRECTION.md)）撰寫，每節經 Codex audit 至 blocking=0（2026-06）。
...
**Status**: manuscript coverage complete（§3.1–§3.3，handout，2026-06-08）——三節全部通過 six-stage convergence gate，包含 two-model adversarial audit（Claude multi-lens + Codex gpt-5.5 xhigh）。
