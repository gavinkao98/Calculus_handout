...
3. **視覺層**：多模態看得到文字審原理上看不到的 render 缺陷（label 碰撞／灰階），**而且
   能自動把圖改好**（碰撞移除、紅線順手改虛線）。
4. **貫穿教訓**：模型迴圈能加速，但文字層 run-to-run 會飄、兩層都需要人 triage、格式交 linter、
...
| figure_critic | 多模態挑圖（§1.1 preview） | 抓到 `h(x)` 撞 y 軸（**每跑必中**）＋灰階 |
| figure_fix | 視覺修圖閉環（reflection 圖） | run2：碰撞**自動修掉**＋紅線改虛線 |
...
- **視覺審查線**：早先「單跑不可靠／run-to-run 飄」的視覺審讀數，使用者 2026-06-15 確認為**誤判**、已撤除。
  視覺審查改以 HTML 線的 figure-audit subagent（看 render 後的 PNG、依 §10 出 defects）承接；詳見 handout。

---

## 5. 操作備忘（換機／下次接續）

- **下一階段計畫（訂閱制：Codex CLI ↔ Claude Code review 迴圈）見
  [`PLAN_codex_subscription_loop.md`](PLAN_codex_subscription_loop.md)** —— 把
  API↔API 換成 CLI↔CLI、走訂閱不走 API（省錢）；本檔教訓全帶過去。
