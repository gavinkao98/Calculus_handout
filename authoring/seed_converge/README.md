...
§1.1（[`legacy/tex_handout/chapters/ch01_foundations.tex`](../../legacy/tex_handout/chapters/ch01_foundations.tex) 第 16–268 行）評分；該 TeX 原稿已隨 LaTeX→HTML 轉移搬進 `legacy/tex_handout/`，本凍結實驗即對照此 legacy 稿評分。
...
python authoring/seed_converge/run.py --dry-run
...
python authoring/seed_converge/run.py --confirm --smoke

# 4) 跑完整 bounded 迴圈（預設 ≤4 輪）
python authoring/seed_converge/run.py --confirm
```

模型 id 走旗標、`--dry-run` 會印出來確認（預設值未必是當下的確切 id）：

```powershell
python authoring/seed_converge/run.py --drafter openai:gpt-5.1 --auditor gemini:gemini-2.5-pro --confirm
```
...
```powershell
python authoring/seed_converge/figure_critic.py --pdf <pdf> --pages 2,6 --dry-run
python authoring/seed_converge/figure_critic.py --pdf <pdf> --pages 2,6 --confirm
```

§1.1 preview 實跑（pages 2,6、~$0.005/run）：**穩定抓到 `h(x)` 標籤撞 y 軸（每跑
必中）** 與灰階只靠顏色編碼等真缺陷。用 `json_schema` 強制結構化（裸 `json_object` 會讓模型脫稿成 bbox／空 list；
...
```powershell
python authoring/seed_converge/figure_fix.py --fig <one-figure.tex> --confirm
```

reflection 圖實跑：**真碰撞（`y=x³` 標籤撞 y 軸）被自動修掉、紅曲線還順手改虛線
（灰階加分）**——證明圖能自動改好。工程上要注意：thinking model 不給足 `--max-tokens`
...
> 註：本實驗凍結於 LaTeX→HTML 轉移之前；當時的「正式講義」即 `chapters/*.tex`，
> 該 TeX 樹現已搬進 `legacy/tex_handout/`，正式講義改為 `handout/` 下的 HTML 片段。
