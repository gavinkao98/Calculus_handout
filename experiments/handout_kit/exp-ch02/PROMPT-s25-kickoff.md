...
## 動手前依序讀（建立脈絡）
1. experiments/handout_kit/CONTRACT-html-writing.md — HTML 寫作契約（主規範；env 詞彙在此）
2. experiments/direction_layer/RULE.md — 六階流程＋方向 brief 模板（§2 九欄）。
   **務必讀 §2「worked example 清單」列的「自創題政策」**（見下「題目政策」）。
3. experiments/handout_kit/exp-ch02/sec-2-1.html、sec-2-2.html、sec-2-3.html、sec-2-4.html —
   四個完成的校準成品。**特別照 sec-2-4.html**（同為規則＋證明密集節：多 Theorem＋多 proof＋
   Caution＋worked example 的可讀性處理）與 sec-2-3.html（定理＋證明結構）。
4. experiments/handout_kit/exp-ch02/brief_s22.md、brief_s23.md、brief_s24.md — brief 範例
   （九欄怎麼填；s24 最近、最貼近本節的「規則密集」型態）。
5. experiments/seed_converge/rules.md — register（語氣／密度，與 LaTeX 版共用）。
...
7. seed：experiments/handout_kit/exp-ch02/seed_ch02_s5.md（手稿 pp. 11–13）。
...
- 截圖自驗：node experiments/handout_kit/_render/shot.mjs
  "http://localhost:8753/chapter2-screen.html" experiments/handout_kit/_render/s25 full
...
## 硬約束（CLAUDE.md）
- 只改 experiments/handout_kit/exp-ch02/、chapter2 HTML、以及策略／紀錄文檔（RULE.md、briefs、
