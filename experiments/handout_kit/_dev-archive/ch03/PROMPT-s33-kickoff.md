...
## 動手前依序讀（建立脈絡）
1. experiments/handout_kit/exp-ch03/PLAN-ch03.md — **ch03 章層方向錨**。**最重要，先讀**（§1 手稿↔ROADMAP、§2 §3.3 範圍、決策 D8/D9/D10、§5 編號 ledger）。
2. experiments/handout_kit/exp-ch03/seed_ch03_s1.md — §3.1 seed（seed 語法範本）。
3. experiments/handout_kit/exp-ch03/sec-3-1.html、sec-3-2.html — §3.1/§3.2 成品（風格＋**編號接續來源**：讀 sec-3-2.html 末尾）。§3.2 的 chain rule（Theorem 號）、§3.1 的 cos′／tan′ 是本節常引用的對象。
4. experiments/handout_kit/CONTRACT-html-writing.md — HTML 寫作契約（**特別 strategy box `<ol class="steps">`**）。
5. experiments/direction_layer/RULE.md — 六階流程＋方向 brief 九欄；**①「seed 轉錄語法」＋ §2「worked example 清單／自創題政策」務必讀**。
6. experiments/seed_converge/rules.md — register。
...
- 渲染：python -m http.server 8753 --bind 127.0.0.1（在 experiments/handout_kit/）；
  node experiments/handout_kit/_render/shot.mjs "http://localhost:8753/chapter3-screen.html" experiments/handout_kit/_render/s33 full。
