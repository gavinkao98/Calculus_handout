...
2. experiments/handout_kit/ch01_example-supplement-review.html  ← 被審物：候選清單＋擬改寫全文
   （HTML 內數學是 MathJax 原始 TeX，直接讀 \( \) 與 \[ \] 即可）
3. experiments/handout_kit/example-ch01/sec-1-1.html … sec-1-6.html  ← 課文現況（缺口分析的基準）
...
E. **不要做**：不評語域用詞品味（最多 advisory）；不建議「多加題目」（數量克制
   是刻意設計：每節 1–3、總量 15+6）；不審尚未發生的插入／編號（那是 import 後
   另一輪的事）；不改寫任何內容。

## Findings 契約（與 RULE.md ⑤ 一致）
- **blocking**＝｛數學錯誤｜對症性不成立（缺口誤判或候選不對題）｜來源/授權標示不實｝
