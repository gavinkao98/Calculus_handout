# CLAUDE.md — handout_kit 子目錄指引

本檔案補充根目錄 [`../../CLAUDE.md`](../../CLAUDE.md) 的專案層級指引，僅涵蓋 `experiments/handout_kit/` 特有的架構約束。

## Fragment 架構：改課文一律改 fragment

課文內容存放在 `fragments/ch{NN}/sec-*.html`，由 `build.py` 組裝成 `chapter{N}-standalone.html`（螢幕版）與 `chapter{N}-print-standalone.html`（列印版）。
...
- **新增圖表需要改兩處：** fragment（加 `<figure>` 元素）和兩個 standalone（加 FIGS 函數）。
