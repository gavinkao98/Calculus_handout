...
（prompt 內檔案路徑為相對 repo 根）：
    ...
    cmd /c "%LOCALAPPDATA%\OpenAI\Codex\bin\codex.exe -c service_tier=fast exec --sandbox read-only - < experiments\handout_kit\_audit\PROMPT-ch{NN}-prose-audit.md > experiments\handout_kit\_audit\REPORT-ch{NN}-prose-audit.raw.txt 2>&1"
...
{{SECTIONS}}  要審的 fragment 路徑（可多個），如
              experiments/handout_kit/fragments/ch01/sec-1-1.html … sec-1-6.html
-->
