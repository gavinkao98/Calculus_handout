...
3. 接著進入第一節（各節為 `handout/fragments/chNN/sec-*.html` 片段）。

實際 HTML 標記（標題層級、清單、技能區塊）見 [`handout/_dev-archive/general/CONTRACT-html-writing.md`](handout/_dev-archive/general/CONTRACT-html-writing.md) 與 [`handout/TYPESETTING_GUIDE.md`](handout/TYPESETTING_GUIDE.md)。

章節結尾 **MUST** 有一個 Summary 區段，包含 definitions / theorems / formulas 三個區塊。見 spec §4。
