...
## 2. 全書現況（canonical 重測，2026-07-25）
...
| **ch04** | 16 | 6,967 | **2.3** | 68 | 43 | 82 | 27 | ✅ 已達標（原 13.3；2026-07-26 合併 sweep 執行 97 條）——剩下 16 個＝逐例通過四步仲裁的成對插入語（兩閘一致），其中 §4.3 為 raw 6/566（N<1000，依 SPEC 不單獨判定）；紀錄 [`REVIEW-ch04-plain-applied.html`](REVIEW-ch04-plain-applied.html) |
| ch03 | 54 | 3,855 | 14.0 | 36 | 20 | 60 | 28 | ⏳ 待做（~~舊尺 8.0~~） |
| ch05 | 108 | 7,502 | 14.4 | 77 | 47 | 43 | 28 | ⏳ 待做（~~舊尺 10.7~~） |
| ch02 | 116 | 6,942 | 16.7 | 58 | 43 | 66 | 21 | ⏳ 待做（~~舊尺 11.1~~） |
| ch07 | 154 | 8,795 | 17.5 | 102 | 37 | 56 | 32 | ⏳ 待做（~~舊尺 13.2~~） |
| appC | 24 | 1,347 | 17.8 | 8 | 15 | 7 | 3 | ⏳ 待做；**N<1000 附近，報 raw 24/1347**（~~舊尺 11.4~~） |
| appA | 58 | 3,081 | 18.8 | 32 | 24 | 18 | 8 | ⏳ 待做，全書最高（~~舊尺 12.8~~） |

**目標值 `T_can` ≤ 3.0/1000（canonical stream）。** 依據＝canonical 重測的真實教材基準（§1）：mooculus 0.0、APEX V5 0.5、CLP1 3.1。目標貼 CLP1（五本中最口語者）的上緣；已達標單元為 **appB 2.2** 與 **ch01 2.1**，兩者都落在 APEX 與 CLP1 之間。
...
| 2 | ch02 | 95 | |
| 3 | ch05 | 86 | |
| ~~4~~ | **ch04** | ~~71~~ | ✅ **2026-07-26 完成**。執行 97 條（gate 1 走查 84 → Codex gate-2：42 ADOPT／41 MODIFY／1 REJECT／13 new → triage）、13.3→2.3/1000、四項 tic guard 無一上升（括號 82 持平為全書最高）、段落觸發器 4→0、HTML 分頁 41 頁不變。**Gate 0 推翻前提**：ch04 的 8-gram 逐字覆蓋率僅 1.6%（ch01 為 34.7%），散文其實是依手稿重寫的 LLM 散文，非手稿逐字。**Gate 7（LaTeX）四閘全綠、成品已產出**：`NAMES` 補 ch04、三個方言差集（數學區段內的 `&lt;`／`&gt;` entity 未解碼、裸 `span.qed`、`env-corollary`）、6 panel 圖匯出；字形閘為此新增 glyf（FontFile2）比對路徑（圖裡的 web NewCM 是 TrueType-flavored）。`dist/ch04/` ＝ chapter4.tex ＋ chapter4.pdf（32 頁）。另補正 3 處數學嚴謹度缺口。紀錄 [`../../latex/chapters/ch04/DIALECT-ch04.md`](../../latex/chapters/ch04/DIALECT-ch04.md) |
