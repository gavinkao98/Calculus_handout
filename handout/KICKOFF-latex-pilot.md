...
> `_review/REVIEW-latex-pilot-appB.html`（對照報告，含三輪 Codex gate-2 紀錄與處置）。
> 工程要點（rollout 沿用）：needspace 套件的誘餌斷點病理改 `\cb@needspace` 硬檢查
> （calcbook.sty 註解有 `\tracingpages` 證據）；style 白名單收口 parser 層＋重複屬性硬錯；
> M-B0 對 §1 初步盤點的更正見 `DIALECT-appB.md` §6。v1 `shell/` 已歸檔
> `../_dev-archive/tex_export-v1-shell/`。後續＝§9 rollout（屆時另開計畫）。

> **給新 session 的開場**：本檔是 2026-07-15/16 兩輪討論的完整交接（v2）。接手流程——
> ① 先讀本檔全文；② 再讀 [`CONTRACT-html-writing.md`](CONTRACT-html-writing.md)（輸入方言契約）、
> [`TYPESETTING_GUIDE.md`](TYPESETTING_GUIDE.md) §9（版心／字體拍板）、[`../CLAUDE.md`](../CLAUDE.md)（行為準則，尤其「缺套件先問」「commit 需授權」）、
> `tex_export/DIALECT-ch03.md`（v1 盤點格式範例）；③ 從 **M-B0** 開始按 §5 推進。
...
- **v1（converter-first，ch03）**：2026-07-15 grilling 拍板、07-16 凌晨另一 session 執行，**已做到 review 階段**——產物全在 `tex_export/`：`convert.py`（IR＋emitter＋單趟數學 scanner）、`test_convert.py`（605 段數學逐位元組驗證）、`dialect_inventory.py`、`check_prose.py`（pdftotext 散文子序列閘）、`export_figs.mjs`＋`figs/ch03/`、`shell/`（book class preamble）、`build/chapter3.pdf`、`_review/REVIEW-latex-pilot-ch03.html`（對照報告）。
...
4. 對照報告（HTML render vs PDF 抽樣並排，standalone HTML，照「打開就能讀」慣例；範本＝`_review/REVIEW-latex-pilot-ch03.html`）。
...
```
handout/tex_export/
  convert.py            # v1 資產：IR＋emitter＋數學 scanner（M-B2 重定向 emitter）
  test_convert.py       # v1 資產：golden tests＋605 段數學逐位元組驗證（M-B2 擴 appB）
  dialect_inventory.py  # v1 資產：方言盤點器（M-B0 跑 appB）
  check_prose.py        # v1 資產：pdftotext 散文子序列閘（M-B3 跑 appB）
  export_figs.mjs       # v1 資產：圖匯出（本 pilot 不動；rollout 首個有圖章驗收）
  DIALECT-ch03.md       # v1 盤點（參照）；M-B0 新增 DIALECT-appB.md
  template/             # v2 新增：模板（樣式層＋語意指令層，M-B1 產出）
    sampler.tex         #   手寫 showcase（拍板用）
  shell/                # v1 的 book-class shell（被 template/ 取代後歸檔或刪，M-B4 清）
  figs/ch03/            # v1 圖匯出樣張（保留參照）
  build/                # lualatex 工作目錄（gitignored；內含 v1 的 chapter3.pdf 參照）
  _review/              # 對照報告（v1 的 ch03 版在此；M-B3 產 appB 版）
```
...
appB 無圖。`export_figs.mjs`＋`figs/ch03/` 保留原樣；正式驗收移至 rollout 首個有圖章（屆時沿 v1 的 Chrome print-to-pdf 路線，fallback 需新安裝先問）。
...
| M | 內容 | 驗證點 |
|---|------|--------|
| M-B0 | appB 方言盤點：`dialect_inventory.py` 跑 appB → `DIALECT-appB.md`（含與 ch03 差集、語意指令需求清單） | 盤點落檔；差集清單凍結 |
...
- **commit 紀律**：經使用者授權才 commit；訊息繁中、body 記裁決。目前整個 `tex_export/`＋本檔均未 commit——**建議 M-B0 前先請使用者授權 commit 一次 v1＋v2 kickoff 快照**（換機保險）。
...
*v1：2026-07-15 grilling session 產出（converter-first，ch03）。v2：2026-07-16 使用者裁決改道（模板先行＋appB＋互動拍板），同日更新本檔。實測證據（CDP 探針、hyphen-data 對照、shell 漂移 diff）見 2026-07-15 對話；v1 執行產物見 `tex_export/`。*
