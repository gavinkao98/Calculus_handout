...
% ============================================================
% chapter6.tex — ch06 出版成品（自動產生，勿手改）
% 產生：cd handout/latex && python make_dist.py ch06
%   內容源＝handout/html/fragments/ch06/（fragment 唯讀；數學逐位元組 pass-through）
...
We rely on this guarantee here so that the definition gives one unambiguous value, and the next section states the theorem precisely. Its proof, which uses the fact that a continuous function on a closed interval changes by only a small amount on sufficiently short subintervals, is given in the Proof-Track appendix.
