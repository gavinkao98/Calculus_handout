...
% ============================================================
% chapter7.tex — ch07 出版成品（自動產生，勿手改）
% 產生：cd handout/latex && python make_dist.py ch07
%   內容源＝handout/html/fragments/ch07/（fragment 唯讀；數學逐位元組 pass-through）
...
Definition 7.2 never mentions revolution. Definition 7.2 applies to any solid whose cross-sectional area we can describe, and for many classical solids the description is a matter of similar triangles.
...
Let \(f\) be continuous with \(f \ge 0\) on \([a, b]\), where \(0 \le a < b\), and rotate the region under \(y = f(x)\) about the \(y\)-axis. Cut \([a, b]\) into \(n\) strips of width \(\Delta x\) as usual. The strip over \([x_{i-1}, x_i]\), swept around the axis, produces a shell whose inner wall has radius \(x_{i-1}\) and whose outer wall has radius \(x_i\). As in §7.2, we state the modelling assumption first, and state it explicitly. We take the solid’s volume to be the limit of the volumes of these shell stacks as the strips shrink. This is the same assumption, made for the same reason, as the slab model of Definition 7.2, only with the slices now nested around the axis instead of stacked along it.
