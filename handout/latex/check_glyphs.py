...
**這條閘的已知極限（別把它當成它不是的東西）**：
  ...
  - 驗 CFF（FontFile3／CIDFontType0C，比 CID）與 glyf（FontFile2／TrueType，比 GID）
    兩種嵌入格式。glyf 那條路是 2026-07-26 ch04 rollout 加的：圖由 headless Chrome 重繪，
    面板上的襯線標籤用 standalone 的 web New Computer Modern，而那份 woff2 是
    TrueType-flavored，於是圖 PDF 裡是 FontFile2（見 audit_glyf 的說明與
    template/fonts/webcm/README.md）。其餘格式、非 CID-keyed 的 CFF、或找不到原始字型檔，
    一律 **FAIL 並指名**，不默默略過——silent skip 正是 check_prose.py 的
    figure_note_check 記錄過的偽陰性坑，不重蹈。
"""
import io
import re
import subprocess
