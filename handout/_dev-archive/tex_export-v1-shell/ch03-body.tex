\hkchapter{Chapter 3}{Chain Rule and Trigonometric Derivatives}
\begin{hklead}
Chapter 2 built the derivative from the ground up — as the limit of a difference quotient — and turned it into a working toolkit: rules for constants, powers, sums, products, quotients, and the exponential. Yet a whole world of functions still lies just out of reach. How fast does \(\sin x\) change? And what is the derivative of a \emph{composition} such as \(\sqrt{1 + x^{2}}\) or \(\sin(x^{2})\), where one function is fed into another? This chapter supplies the two ideas that complete the differentiation toolkit. First we differentiate the trigonometric functions, which forces us to confront a limit no algebra can crack and to break through it with a piece of geometry. Then we prove the \emph{chain rule}, the single rule that differentiates any composition, and use it to unlock the derivatives of inverse and implicitly defined functions.
\end{hklead}
\hkparahead{By the end of this chapter, you will be able to:}
\begin{itemize}
...
\end{itemize}
\hksection{3.1}{Derivatives of the Sine and Cosine Functions}
So far every derivative has yielded to algebra. For a polynomial we expanded \((x + h)^{n}\) and canceled the factor of \(h\); for \(e^{x}\) the power series did the same job. The trigonometric functions are different in kind. Consider the difference quotient for \(\sin x\), \[ \frac{\sin(x + h) - \sin x}{h}. \] No algebraic identity cancels the \(h\) in the denominator against the numerator — \(\sin(x + h)\) does not break into pieces that conveniently subtract. As we are about to see, the entire computation funnels down to a single limit, \[ \lim_{\theta \to 0} \frac{\sin \theta}{\theta}, \] which has the indeterminate form \(0/0\) and which no amount of algebra will resolve. To evaluate it we need a genuinely new tool: a geometric comparison of areas on the unit circle, squeezed to a limit. That one limit, once secured, unlocks the derivatives of \emph{both} \(\sin\) and \(\cos\) — and through them the rest of the trigonometric functions.

Throughout this section angles are measured in radians; this assumption turns out to be essential, as we note below.

\hksubsection{The difference quotient for sine}
Fix \(x\) and form the difference quotient for \(f(x) = \sin x\). The decisive algebraic step is the sum-to-product identity \(\sin A - \sin B = 2 \cos\frac{A + B}{2} \sin\frac{A - B}{2}\). If this identity is not at your fingertips, it is the angle-sum formulas in disguise: set \(u = \frac{A + B}{2}\) and \(v = \frac{A - B}{2}\), so that \(A = u + v\) and \(B = u - v\); expanding \(\sin(u + v) - \sin(u - v)\) by the angle-sum formulas cancels the \(\sin u \cos v\) terms and leaves exactly \(2 \cos u \sin v\). (Appendix A.2 derives the whole family of such identities.) Applied with \(A = x + h\) and \(B = x\): \[ \sin(x + h) - \sin x = 2 \cos\Bigl(x + \tfrac{h}{2}\Bigr) \sin\frac{h}{2}. \] Dividing by \(h\) and writing the denominator as \(h = 2 \cdot \tfrac{h}{2}\), to match the \(\tfrac{h}{2}\) inside the sine, gives
...
\hksubsection{A geometric inequality on the unit circle}
To get a grip on \(\sin\theta/\theta\) for small \(\theta\), we compare three areas on a circle of radius \(1\). Because both \(\theta\) and \(\sin\theta\) reverse sign when \(\theta\) is replaced by \(-\theta\), the ratio \(\sin\theta/\theta\) is unchanged — it is an \emph{even} function — so it is enough to treat \(\theta > 0\), shrinking to \(0\). Take \(0 < \theta < \tfrac{\pi}{2}\) and look at Figure 3.1.

\begin{hkfigure}
  \includegraphics[width=79.9mm]{ch03/sector-inequality}
\hkfigcaption{Figure 3.1}{The unit-circle area comparison behind \(\sin\theta \le \theta \le \tan\theta\).}
\end{hkfigure}
...
\end{hkproof}
\hksubsection{The fundamental limit}
With continuity secured, we can finally squeeze \(\sin\theta/\theta\) to its limit (Figure 3.2).

\begin{hkproposition}{Proposition}{3.2}{The fundamental trigonometric limit}
...
\end{hkproof}
\begin{hkfigure}
  \includegraphics[width=81.92mm]{ch03/squeeze-limit}
\hkfigcaption{Figure 3.2}{A graphical reading of the squeeze in Proposition 3.2: \(\dfrac{\sin\theta}{\theta}\) sits between \(\cos\theta\) and \(1\), with an open circle at \(\theta = 0\).}
\end{hkfigure}
\begin{hkcaution}{Caution}{}{}
...
\end{hkcaution}
\begin{hkcaution}{Caution}{}{}
Everything here depends on measuring angles in \emph{radians}. The sector area \(\tfrac12\theta\) — and with it the limit \(\sin\theta/\theta \to 1\) — uses the radian measure, for which the arc cut off on the unit circle has length exactly \(\theta\). In degrees the limit is not \(1\): since \(x^{\circ} = \tfrac{\pi}{180}x\) radians, one finds \(\lim_{x \to 0}\dfrac{\sin(x^{\circ})}{x} = \dfrac{\pi}{180}\), and so \(\dfrac{d}{dx}\sin(x^{\circ}) = \dfrac{\pi}{180}\cos(x^{\circ})\). The clean formulas of this section, \(\sin' = \cos\) and \(\cos' = -\sin\), hold only in radians.
\end{hkcaution}
\hksubsection{The derivatives of sine and cosine}
With continuity and the fundamental limit in hand, the difference quotient for \(\sin\) all but evaluates itself.

\begin{hktheorem}{Theorem}{3.1}{Derivative of sine}
...
\end{hkproof}
Cosine yields to the very same machinery, this time through the companion identity \(\cos A - \cos B = -2 \sin\frac{A + B}{2} \sin\frac{A - B}{2}\) — obtained just like the sine version, by expanding \(\cos(u + v) - \cos(u - v)\) with the angle-sum formulas.

\begin{hktheorem}{Theorem}{3.2}{Derivative of cosine}
...
\end{hkproof}
\begin{hkfigure}
  \includegraphics[width=55.07mm]{ch03/sin-cos-slope}
\hkfigcaption{Figure 3.3}{Tangent slopes on \(y = \sin x\) matching the heights of \(y = \cos x\) at the same \(x\)-values.}
\end{hkfigure}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.1}{}
Establish the companion limit \(\displaystyle \lim_{\theta \to 0} \frac{1 - \cos\theta}{\theta} = 0\).
\end{hkexample}
\begin{hksolution}{Solution}{}{}
This is again of the form \(0/0\). Multiplying numerator and denominator by \(1 + \cos\theta\) converts the numerator into something \(\sin\) can handle:
...
where the Pythagorean identity \(\cos^{2} x + \sin^{2} x = 1\) collapses the numerator.
...
Thus \(s''(t) = -\sin t = -s(t)\): at every instant the weight accelerates back toward its rest level, by an amount proportional to how far it has strayed. This relation \(s'' = -s\) is the signature of \emph{simple harmonic motion} (Figure 3.4), and it is no accident that sine and cosine — the functions whose second derivative is their own negative — are precisely the ones that describe oscillation.
\end{hksolution}
\end{hkworkedexample}
\begin{hkfigure}
  \includegraphics[width=79.9mm]{ch03/shm-triple}
\hkfigcaption{Figure 3.4}{Simple harmonic motion over one time axis: \(s = \sin t\), \(s' = \cos t\), and \(s'' = -\sin t\).}
\end{hkfigure}
\begin{hkremark}{Remark}{3.1}{The derivative cycle of sine and cosine}
Differentiation sends sine and cosine into each other, with a sign that turns over every second step (Figure 3.3). Writing \(\to\) for one application of \(\dfrac{d}{dx}\),

\[ \sin x \;\to\; \cos x \;\to\; -\sin x \;\to\; -\cos x \;\to\; \sin x. \]

After four derivatives we are back where we started, so \(\dfrac{d^{4}}{dx^{4}}\sin x = \sin x\), and likewise for cosine. Where \(e^{x}\) reproduces itself in a single step (§2.4), sine and cosine reproduce themselves in a cycle of four — the same self-renewal that lets \(s'' = -s\) describe an oscillation that never winds down.
\end{hkremark}
...
\begin{hktheorem}{Theorem}{3.3}{The chain rule}
...
\end{hktheorem}
\begin{hkfigure}
  \includegraphics[width=79.9mm]{ch03/composed-mapping}
\hkfigcaption{Figure 3.5}{The chain rule as a composed mapping: local slope factors accumulate along \(x \mapsto u \mapsto y\).}
\end{hkfigure}
\begin{hkremark}{Remark}{3.2}{The Leibniz form}
Writing \(u = g(x)\) for the inner variable and \(y = f(u)\) for the output, the chain rule takes the memorable Leibniz shape

\[ \frac{dy}{dx} = \frac{dy}{du}\cdot\frac{du}{dx}. \]

This is the form most often used in practice, and it reads as “the rate of \(y\) per \(x\) equals the rate of \(y\) per \(u\) times the rate of \(u\) per \(x\).” The apparent cancellation of \(du\) is a useful memory aid, but it is \emph{not} a proof: \(\dfrac{dy}{du}\) and \(\dfrac{du}{dx}\) are limits, not fractions with a common factor to strike out. What actually makes the rule true is the argument given below, where the controlled error terms — not a literal \(du/du = 1\) — do the work.
\end{hkremark}
\begin{hkstrategy}{Strategy}{3.1}{Differentiating a composition}
...
\begin{hksteps}
  ...
  \item Differentiate the outer function at the inner one, forming \(f'(g(x))\) — treat the inside as a single block and leave it untouched for now.
...
\begin{hkinformal}
Informally, \(f(x_{0}+h)\) equals its tangent-line value \(f(x_{0}) + m h\) plus an error \(R(h)\) that dies away faster than \(h\) does — the tangent line is a good fit to first order (Figure 3.6).
\end{hkinformal}
\end{hkdefinition}
\begin{hkfigure}
\includegraphics[width=65.62mm]{ch03/remainder-tangent-1}\hspace{6mm}\includegraphics[width=65.62mm]{ch03/remainder-tangent-2}
\hkfigcaption{Figure 3.6}{Remainder-form geometry: halving \(h\) makes the vertical error \(R(h)\) much smaller, illustrating \(R(h)/h \to 0\).}
\end{hkfigure}
This is not a new notion of derivative, only a new way of writing the old one.

\begin{hkproposition}{Proposition}{3.3}{Equivalence of the two forms}
...
\end{hkproof}
With the remainder form available, the chain rule comes out by substitution. We expand \(g\) near \(x_{0}\) and \(f\) near \(g(x_{0})\), each as a linear part plus a small remainder; feeding one into the other, the linear parts multiply to give \(g'(x_{0})f'(g(x_{0}))\,h\), and everything left over gathers into a single remainder. The whole proof is then the one estimate that this leftover still vanishes faster than \(h\).

\begin{hkproof}{Proof}{}{of Theorem 3.3}
...
\end{itemize}
Now fix \(h\) with \(0 < \lvert h \rvert < \alpha\). If \(m_{1} h + R_{1}(h) = 0\), then \(R_{2}(m_{1} h + R_{1}(h)) = R_{2}(0) = 0\) — the remainder relation \(f(g(x_{0}) + y) = f(g(x_{0})) + m_{2} y + R_{2}(y)\) at \(y = 0\) forces \(R_{2}(0) = 0\) — and the quotient is \(0\). Otherwise \(0 < \lvert m_{1} h + R_{1}(h) \rvert < \delta\), and we factor
...
\end{hkproof}
\hksubsection{Using the rule}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.4}{}
...
The chain rule does not finish the job by itself: the factor it asks for — the derivative of the inside — is a small problem of its own, here a quotient rule.
\end{hksolution}
\end{hkworkedexample}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.7}{}
...
The remaining derivative needs the chain rule, since \(\cos^{2} x = [\cos x]^{2}\) is a composition — outer function \((\,\cdot\,)^{2}\), inner function \(\cos x\):
...
\end{hkworkedexample}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.8}{}
In a harbour, the amount of kelp \(K\) depends on the urchin population \(U\) — urchins eat kelp — and \(U\) in turn depends on the otter population \(O\), since otters eat urchins. Use the chain rule to decide whether \(\dfrac{dK}{dO}\) is positive or negative.
\end{hkexample}
\begin{hksolution}{Solution}{}{}
Read off the sign of each link first. More urchins means less kelp, so \(K\) decreases as \(U\) increases: \(\dfrac{dK}{dU} < 0\). More otters means fewer urchins, so \(\dfrac{dU}{dO} < 0\). The chain rule in Leibniz form (Remark 3.2) chains the two rates:

\[ \frac{dK}{dO} = \frac{dK}{dU}\cdot\frac{dU}{dO}. \]

A product of two negative numbers is positive, so \(\dfrac{dK}{dO} > 0\): more otters ultimately means more kelp. This is the formal version of the two-step argument — otters thin out the urchins, and fewer urchins let the kelp recover — with each \(dy/du\) factor carrying the sign of one link and the signs multiplying along the chain.
\end{hksolution}
\end{hkworkedexample}
With the chain rule in hand the differentiation toolkit is essentially complete: any function assembled by composing the elementary functions of Chapters 2 and 3 can now be differentiated mechanically. In the next section the rule does something new — it becomes the key that unlocks derivatives we could not otherwise reach, those of the inverse functions \(\ln x\), \(\arcsin x\), and \(\arctan x\), and, through logarithmic differentiation, of expressions such as \(x^{x}\).


\hksection{3.3}{Applications of the Chain Rule}
Sections 3.1 and 3.2 built the machinery; this section spends it. The chain rule (Theorem 3.3) was advertised as the tool that completes the differentiation toolkit, and here we see why: it reaches a whole family of derivatives that no rule of Chapter 2 can touch directly. We have no formula for \(\ln x\) to differentiate — it is known only as the function that undoes \(e^{x}\). The expression \(x^{x}\) carries its variable in \emph{both} the base and the exponent, so neither the power rule nor the rule for \(e^{x}\) applies to it. And \(\arcsin\) has no elementary formula at all. Each of these functions is defined only indirectly.

Yet each one satisfies an \emph{identity} assembled from functions we already know how to differentiate: \(e^{\ln x} = x\), or \(\ln(x^{x}) = x\ln x\), or \(\arcsin(\sin x) = x\) for \(x \in \bigl[-\tfrac{\pi}{2}, \tfrac{\pi}{2}\bigr]\). That is the whole idea of this section, used over and over: \emph{to differentiate a function you cannot attack head-on, differentiate an identity it satisfies, then solve for the derivative you want.} It is the chain rule run backward — instead of pushing a known derivative forward through a composition (Strategy 3.1), we apply the chain rule to a known relation and read off the one derivative we do not yet have. The same move handles the inverse functions \(\ln\), \(\arcsin\), \(\arccos\), and \(\arctan\), and, through the closely related trick of \emph{logarithmic differentiation}, awkward powers like \(x^{x}\).

\hksubsection{The logarithm and logarithmic differentiation}
We begin with the natural logarithm. Throughout this section we use \(\ln x\) informally, as the inverse of the exponential function — the number whose exponential is \(x\), so that \(e^{\ln x} = x\) for every \(x > 0\). This relation is all we need to find its derivative. The rigorous construction of \(\ln x\) as the inverse of \(e^{x}\), together with a proof that it is differentiable, is deferred to Chapter 4 (§4.5), which re-derives the formula below from the ground up.

\begin{hkworkedexample}
\begin{hkexample}{Example}{3.9}{}
Show that \(\dfrac{d}{dx}\ln x = \dfrac{1}{x}\) for \(x > 0\).
\end{hkexample}
\begin{hksolution}{Solution}{}{}
Start from the identity that defines the logarithm, \(e^{\ln x} = x\), and differentiate both sides with respect to \(x\). The left side is a composition — the outer function is \(\exp\), the inner is \(\ln x\) — so by the chain rule (Theorem 3.3), using \(\dfrac{d}{du}e^{u} = e^{u}\) (Chapter 2, §2.4, Theorem 2.5),
...
The unknown derivative was trapped inside a relation we could differentiate; one application of the chain rule and a division set it free.
\end{hksolution}
\end{hkworkedexample}
The derivative of the logarithm is more than one more entry in a table — it powers a technique. When a function carries its variable in the exponent, or is a sprawling product and quotient of many factors, taking its logarithm first turns powers into products and products into sums, all of which we can already differentiate. Applying \(\dfrac{d}{dx}\ln\) to both sides and then solving is called \emph{logarithmic differentiation}.

\begin{hkstrategy}{Strategy}{3.2}{Logarithmic differentiation}
...
The derivative is exactly \(\ln x\) — so \(x\ln x - x\) is an \emph{antiderivative} of \(\ln x\), a fact worth filing away for when we turn to integration.
\end{hksolution}
\end{hkworkedexample}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.13}{}
...
Each factor of the original product contributes exactly one term to the bracket: logarithmic differentiation trades one hard product-rule computation for several easy logarithmic derivatives added together.
\end{hksolution}
\end{hkworkedexample}
\hksubsection{Derivatives of the inverse trigonometric functions}
The logarithm gave way because it is the inverse of \(e^{x}\): we differentiated the relation \(e^{\ln x} = x\). The inverse trigonometric functions yield to exactly the same lever — only now the identity comes from trigonometry. Each inverse function composes with its parent to give an identity, and differentiating that identity by the chain rule isolates the derivative we are after.

\begin{hkworkedexample}
\begin{hkexample}{Example}{3.14}{}
...
\end{hkcaution}
\begin{hkfigure}
  \includegraphics[width=64.03mm]{ch03/arcsin-vertical-tangent}
\hkfigcaption{Figure 3.7}{Endpoint behavior of \(\arcsin y\): the tangent becomes vertical at \(y = \pm 1\), so the derivative formula applies only on \((-1, 1)\).}
\end{hkfigure}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.15}{}
...
\end{hkworkedexample}
\begin{hkworkedexample}
\begin{hkexample}{Example}{3.16}{}
Show that \(\dfrac{d}{dy}\arctan y = \dfrac{1}{1 + y^{2}}\) for all \(y \in \mathbb{R}\).
\end{hkexample}
\begin{hksolution}{Solution}{}{}
Once more the same lever. Recall that \(\arctan\) is the inverse of tangent on the branch \(\bigl(-\tfrac{\pi}{2}, \tfrac{\pi}{2}\bigr)\), so \(\arctan(\tan x) = x\). Differentiating with respect to \(x\), and using \(\dfrac{d}{dx}\tan x = \sec^{2} x\) (Example 3.2),
...
\end{hksolution}
\end{hkworkedexample}
\begin{hkremark}{Remark}{3.3}{One lever for many derivatives}
Look back at what just happened. We never differentiated \(\ln x\), \(x^{x}\), or an inverse trigonometric function \emph{directly} — there was no rule to do so. In every case we found a known identity the function satisfies — \(e^{\ln x} = x\), \(\ln(x^{x}) = x\ln x\), \(\arcsin(\sin x) = x\) on its branch, and their kin — differentiated it by the chain rule, and solved for the one unknown. The chain rule is not only a tool for pushing derivatives forward through a composition; turned on an identity, it is a lever that pries loose derivatives we could reach in no other way.
\end{hkremark}
\hksubsection{Chapter summary}
...
With these, Chapter 3 is complete. It opened by breaking the one limit algebra could not touch, \(\lim_{\theta \to 0}\dfrac{\sin\theta}{\theta}\), with a piece of unit-circle geometry, and read off \(\sin' = \cos\) and \(\cos' = -\sin\) (§3.1). It then proved the chain rule, the rule for compositions that Chapter 2 had been missing (§3.2). Here that rule was turned loose to reach the derivatives of the logarithmic, exponential, and inverse trigonometric functions (§3.3). Together with the algebraic rules of Chapter 2, we can now differentiate essentially every function built by combining and composing the elementary ones.

Two debts remain, and both were taken on informally. We have used \(\dfrac{d}{dx}e^{x} = e^{x}\) from the casual series argument of Chapter 2, and we have treated \(\ln x\) as the inverse of \(e^{x}\) without ever constructing it. Chapter 4 settles both: it builds \(e^{x}\) rigorously and re-derives its derivative (§4.3), and it proves the Mean Value Theorem (§4.4) — the result that finally lets us define \(\ln x\) as the inverse of the strictly increasing exponential and recover \(\dfrac{d}{dx}\ln x = \dfrac{1}{x}\) on solid ground (§4.5).
