...
//
//   node handout/_render/linebreak-gate.mjs                 # 檢查全部四章 standalone
//   node handout/_render/linebreak-gate.mjs <file.html> ... # 只檢查指定檔
//
...
// 預設目標：handout/ 下的四章 print-standalone（本檔在 handout/_render/，故往上一層）。
const HERE = dirname(fileURLToPath(import.meta.url));
const HANDOUT = resolve(HERE, "..");
const args = process.argv.slice(2);
const targets = args.length
  ? args.map((p) => resolve(process.cwd(), p))
  : [1, 2, 3, 4].map((n) => resolve(HANDOUT, `chapter${n}-print-standalone.html`));
