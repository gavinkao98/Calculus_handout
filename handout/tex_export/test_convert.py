...
class MathPassthrough(unittest.TestCase):
    ...
    def test_every_ch03_math_segment_is_byte_identical_in_order(self):
        ...
        """
        tex, _ = convert_chapter("ch03", Path(__file__).parent / "figs" / "ch03" / "figures.json")
        src_math = []
        for f in ("sec-3-1", "sec-3-2", "sec-3-3"):
            src_math += naive_scan_math(
                (HANDOUT / "fragments" / "ch03" / f"{f}.html").read_text(encoding="utf-8"))
...
class AppBMappings(unittest.TestCase):
    ...
    def test_every_appB_math_segment_is_byte_identical_in_order(self):
        ...
        tex, _ = convert_chapter("appB", Path(__file__).parent / "figs" / "appB" / "figures.json")
        src_math = []
        for f in ("sec-b-1", "sec-b-2", "sec-b-3", "sec-b-4", "sec-b-5"):
            src_math += naive_scan_math(
                (HANDOUT / "fragments" / "appB" / f"{f}.html").read_text(encoding="utf-8"))
        self.assertEqual(len(src_math), 317, "數學區段數應為 317＝inline 308＋display 9（M-B0 盤點）")
        pos = 0
        for k, m in enumerate(src_math):
            j = tex.find(m, pos)
            self.assertGreaterEqual(j, 0, f"第 {k} 段數學未依序出現在 .tex：{m[:60]!r}")
    ...
    def test_appB_converts_with_no_hard_errors(self):
        ...
        tex, stats = convert_chapter("appB", Path(__file__).parent / "figs" / "appB" / "figures.json")
        self.assertEqual(stats["mapped"], 440)   # 鎖實值（gate-2 A2：>300 太弱）
        self.assertEqual(stats["math"], 317)
