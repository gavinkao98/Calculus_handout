...
template/M-B1-DECISIONS.md §2；mapping 權威＝DIALECT-ch03.md ＋ DIALECT-appB.md（差集）。
...
class Para:
    kids: list
    variant: str = "normal"          # normal | lead | informal | para-head | center
...
class Builder:
    ...
    def block(self, el):
        ...
        if t == "p":
            ...
            v = c[0]
            if v in ("lead", "informal", "para-head") and len(c) == 1:
                return Para(self.inlines(el.kids, el), variant=v)
            self.err(el, "<p> 只允許無 class 或 lead／informal／para-head")
...
def convert_chapter(ch_id, figs_path):
    ...
    for frag_id in CHAPTERS[ch_id]["fragments"]:
        path = HANDOUT / "fragments" / ch_id / f"{frag_id}.html"
...
def main():
    ...
    figs_path = a.figs or (Path(__file__).parent / "figs" / a.chapter / "figures.json")
    try:
