...
> 拍板日：2026-07-16。設計錨＝現行 HTML 視覺語彙（`appendixB-print-standalone.html` CSS 盤點），依 D9 錨定＋書籍化精緻。
...
| fragment 標記 | 語意指令 |
|---|---|
| 開場（`header.chapter-head`＋kicker＋title） | `\appendixopener{Appendix B}{Reading Theorems and Proofs}`；rollout 章＝`\chapteropener{Chapter 3}{…}` |
