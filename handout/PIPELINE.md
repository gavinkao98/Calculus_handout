...
| Ch2 | ✅ 手稿六階 | ✅ 雙閘／**⬜ 待辦：下次數學閘併入 2 處**（2026-07-26 使用者裁決）——① §2.3「To be differentiable is to have one definite tangent direction」的殘留不精確：`S20` 那句已依 Codex gate-2 改成 finite limit／finite slope 說法，但**下一句**（`The converse fails … where no single tangent direction exists`）帶同樣的問題（\(\sqrt[3]{x}\) 有垂直切線方向卻不可微）；② §2.4 應用段 \(y' = ky\) 宜明說 \(k\) 為常數、並以 \(Ce^{kx}\) 補足「built from \(e^{x}\)」。兩者皆屬數學內容，故未在 ch02 平實化輪執行。證據：[`html/_audit/REPORT-ch02-plain-codex-raw.md`](html/_audit/REPORT-ch02-plain-codex-raw.md) Q5／Q6、[`html/_audit/REVIEW-ch02-plain-applied.html`](html/_audit/REVIEW-ch02-plain-applied.html) §誠實記錄 | ✅／✅ | ✅ gate-1+2 | ✅ | ✅ ①② | **全閘完成** |
