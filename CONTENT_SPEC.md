...
- **類型**欄對應 §10.2 工具選擇：`graph`（pgfplots）、`diagram`（TikZ）、`multi-panel`（§10.3）。
- 佔位符是 LaTeX 註解，不影響編譯；後續 Mode B 稽核時逐條裁決是否繪圖。
...
- [ ] **散文易懂性過閘（HTML 線）**：本章已過散文稽核 gate 1（`handout-prose-audit` subagent），**易懂性 blocking = 0**；契約見 [`experiments/handout_kit/_audit/PROSE-AUDIT-RUBRIC.md`](experiments/handout_kit/_audit/PROSE-AUDIT-RUBRIC.md)。定稿前再經 gate 2（Codex 獨立複核）。
