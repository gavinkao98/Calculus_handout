...
- [`CONTENT_EXERCISES.md`](CONTENT_EXERCISES.md)——完整延後設計前的最低習題骨架。
...
Summary 不引入新內容。它不出現在編號 section 序列中（注意 `\section*`）。它 **SHOULD** 出現在最後一節的 exercise TODO marker 之前（見 §14）。
...
本專案使用恰好 **12 個 environment**。新的章節內容 **MUST** 使用其中之一；**MUST NOT** 引入新 environment 而不更新本文件。
...
| `exercise` | 供學生練習的題目 statement（完整設計延後；見 §14）。 |
...
用於節末 `\subsection*{Exercises}` 區塊中。完整習題系統設計（difficulty marker、hint、answer appendix、inline self-check variant）**延後**至本書主要內容完成後；見 §14。

內容草擬期間，每節結尾 **MUST** 帶一個 placeholder：
...
Exercise 是例外，編號方式不同，因為它們位於 per-section `\subsection*{Exercises}` 區塊中，主要被 locally consume：

- `exercise` → Exercise 1, 2, ...，每 `\section` 重新開始。顯示的編號不帶 chapter 或 section 前綴。見 [`CONTENT_EXERCISES.md`](CONTENT_EXERCISES.md) 中的習題系統 rationale；對特定 exercise 的跨節引用使用下方 §6 的 label + `\cref` 慣例。

Rationale：高中自學讀者翻回去查時會問*「Definition 1.3 在哪裡？」*，期望 "Definition 1.3" 是 Chapter 1 中的**第三個 definition**。Shared counter（本專案早期版本使用）會打破這個期望——"Definition 1.3" 可能前面穿插了 theorem 和 proposition，使編號在查找時 less informative。Per-env chapter-scoped counter 匹配讀者對 formal statement 和 aside environment 的心智模型。Exercise 打破此規則，因為一個剛做完 §1.3 的學生翻到節末習題時想的是*「Exercise 1、Exercise 2、...」*而非*「Exercise 1.3.1、Exercise 1.3.2、...」*；全書前綴在那個 local reading context 中是摩擦，不是資訊。
...
完整習題系統設計（environment variant、difficulty marker、hint、answer appendix、inline self-check variant）**延後**至本書主要內容完成後。對照真實內容設計習題系統將產生比預先對照假設內容設計更好的選擇。

### 內容草擬期間的義務

每節結尾 **MUST** 帶一個 placeholder comment，格式為：

```latex
% TODO: add \subsection*{Exercises} block with end-of-section problems for Section N.M.
```

Placeholder 使缺失的 exercise block 在 source 中可見且可被 CI audit。一節若既無正式 `\subsection*{Exercises}` 區塊也無 TODO placeholder，即不合規。

### 習題設計何時開啟

當大多數章節的主要內容完成（definition、theorem、example、prose），習題系統將在專門的輪次中設計，遵循產出本文件的同一 Q/A 諮詢結構。
...
**習題**
- [ ] 每節結尾有正式 `\subsection*{Exercises}` 區塊或 TODO placeholder。
